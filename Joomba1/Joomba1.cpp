...
bool processa_entrada_joomba_1(std::vector<instrucao>* instrucoes, const int32_t ANDARES, const int32_t JANELAS) {
	...
	int32_t coco = p->janelas_array[12];
	while (!instrucoes->empty()) {
		instrucao i = instrucoes->back();
		switch (i.direcao) {
			case 'D': move_direita(i.distancia, p); break;
			case 'E': move_esquerda(i.distancia, p); break;
			case 'C': move_para_cima(i.distancia, p); break;
			case 'B': move_para_baixo(i.distancia, p); break;
			default: break;
		}
		instrucoes->pop_back();
	}
